// remarkable-color.typ — Pandoc typst template for Kaleido 3 color e-ink
// High-saturation colors for washed-out e-ink gamut

// Pandoc definitions partial
#let blockquote(body) = [
  #block(
    stroke: (left: 3pt + rgb("#006666")),
...
#let conf(
  ...
  // Heading styles — dark blue, bold, clear hierarchy
  show heading.where(level: 1): it => {
    set text(size: 18pt, weight: "bold", fill: rgb("#00008B"))
    block(above: 1.2em, below: 0.6em)[#it.body]
  }
  show heading.where(level: 2): it => {
    set text(size: 14pt, weight: "bold", fill: rgb("#00008B"))
    block(above: 1em, below: 0.5em)[#it.body]
  }
  show heading.where(level: 3): it => {
    set text(size: 12pt, weight: "bold", fill: rgb("#00008B"))
    block(above: 0.8em, below: 0.4em)[#it.body]
  }

  // Code blocks — high saturation syntax colors on light gray
  ...
  if title != none {
    align(center)[#block(inset: (bottom: 1em))[
      #text(weight: "bold", size: 1.5em, fill: rgb("#00008B"))[#title]
    ]]
  }
